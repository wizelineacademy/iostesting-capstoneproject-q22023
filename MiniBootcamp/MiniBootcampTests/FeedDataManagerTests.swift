...
class FeedDataManagerTests: XCTestCase {
  ...
  func testNetworkResponse() {
      let expectation = expectation(description: "Incoming Data")
      var response = false
  ...
  func testResponseWithError() {
      let expectation = expectation(description: "An Error Ocurred")
  ...
  func testResponseWithParsingError() throws {
      let expectation = expectation(description: "Parsing error encountered")
      var expectedError: TweetAPIError?
      session.data = Data()
